...
export default <Resolvers>{
  Query: {
    nextTaskExecution: (_, args) =>
      Timestamp.fromDate(
        getAccountContext().nextExecutionService.get(args.task),
      ),
    nextTasksExecution: () =>
      Timestamp.fromDate(getAccountContext().nextExecutionService.tasks()),
    nextVillageTaskExecution: (_, args) =>
      Timestamp.fromDate(
  ...
  Subscription: {
    nextTaskExecutionChanged: subscribeToEvent(
    ...
    nextTasksExecutionChanged: subscribeToEvent(
    ...
    nextVillageTaskExecutionChanged: subscribeToEvent(
